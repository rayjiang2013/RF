...
class TestlinkAPIClient(object):    
    ...
    def _callServer(self, methodAPI, argsAPI=None):
        ...
        #logger.console('_callServer.methodAPI=%s, argsAPI=%s' % (methodAPI, argsAPI))
        response = None
        try:
            if argsAPI is None:
    ...
    def listProjects(self):
        ...
        projects=self.getProjects()
        for project in projects:
            logger.console("Name: %s ID: %s " % (project['name'], project['id']))
...
if __name__ == "__main__":
    ...
    logger.console(myTestLink)
    logger.console(myTestLink.about())
