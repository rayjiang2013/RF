...
package ixiahltfcoe_fwd_vnport;
##
my @cmd_spec_list = (
    ['fcoe_fwd_vnport_config', 'status', 'log', 'handles'],
    ['fcoe_fwd_vnport_control', 'status', 'log', '*'],
    ['fcoe_fwd_vnport_stats', 'status', 'log', '*'],
);
...
eval {
    ixiatcl::source(File::Spec->join(
        File::Spec->rel2abs(File::Basename::dirname(__FILE__)),
        'fcoe_fwd_vnport.x.tcl'
    ));
}; 
if ($@) {
  Carp::confess("fcoe_fwd_vnport.x.tcl:
----
".ixiatcl::errorInfo()."
------
");
...
print("using " . File::Spec->join(
 File::Spec->rel2abs(File::Basename::dirname(__FILE__)), 'fcoe_fwd_vnport.x.tcl') . "
");
