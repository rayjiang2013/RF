...
package ixiahltptp_over_mac;
##
my @cmd_spec_list = (
    ['ptp_over_mac_config', 'status', 'log', 'handles'],
    ['ptp_over_mac_control', 'status', 'log', '*'],
    ['ptp_over_mac_stats', 'status', 'log', '*'],
);
...
if ($@) {
  Carp::confess("ptp_over_mac.x.tcl:
----
".ixiatcl::errorInfo()."
------
");
...
print("using " . File::Spec->join(
 File::Spec->rel2abs(File::Basename::dirname(__FILE__)), 'ptp_over_mac.x.tcl') . "
");
