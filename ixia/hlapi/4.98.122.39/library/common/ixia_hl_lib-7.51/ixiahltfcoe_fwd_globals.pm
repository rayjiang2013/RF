...
package ixiahltfcoe_fwd_globals;
##
my @cmd_spec_list = (
    ['fcoe_fwd_globals_config', 'status', 'log', 'handles'],
    ['fcoe_fwd_globals_control', 'status', 'log', '*'],
    ['fcoe_fwd_globals_stats', 'status', 'log', '*'],
);
...
eval {
    ixiatcl::source(File::Spec->join(
        File::Spec->rel2abs(File::Basename::dirname(__FILE__)),
        'fcoe_fwd_globals.x.tcl'
    ));
}; 
if ($@) {
  Carp::confess("fcoe_fwd_globals.x.tcl:
----
".ixiatcl::errorInfo()."
------
");
...
print("using " . File::Spec->join(
 File::Spec->rel2abs(File::Basename::dirname(__FILE__)), 'fcoe_fwd_globals.x.tcl') . "
");
