...
package ixiahltfcoe_client_options;
##
my @cmd_spec_list = (
    ['fcoe_client_options_config', 'status', 'log', 'handles'],
    ['fcoe_client_options_control', 'status', 'log', '*'],
    ['fcoe_client_options_stats', 'status', 'log', '*'],
);
...
eval {
    ixiatcl::source(File::Spec->join(
        File::Spec->rel2abs(File::Basename::dirname(__FILE__)),
        'fcoe_client_options.x.tcl'
    ));
}; 
if ($@) {
  Carp::confess("fcoe_client_options.x.tcl:
----
".ixiatcl::errorInfo()."
------
");
...
print("using " . File::Spec->join(
 File::Spec->rel2abs(File::Basename::dirname(__FILE__)), 'fcoe_client_options.x.tcl') . "
");
