...
##
my @cmd_spec_list = (
    ['atm_config', 'status', 'log', 'handles'],
    ['atm_control', 'status', 'log', '*'],
    ['atm_stats', 'status', 'log', '*'],
);
...
print("using " . File::Spec->join(
 File::Spec->rel2abs(File::Basename::dirname(__FILE__)), 'atm.x.tcl') . "
");
