...
package ixiahltethernetrange;
##
my @cmd_spec_list = (
    ['ethernetrange_config', 'status', 'log', 'handles'],
    ['ethernetrange_control', 'status', 'log', '*'],
    ['ethernetrange_stats', 'status', 'log', '*'],
);
...
if ($@) {
  Carp::confess("ethernetrange.x.tcl:
----
".ixiatcl::errorInfo()."
------
");
...
print("using " . File::Spec->join(
 File::Spec->rel2abs(File::Basename::dirname(__FILE__)), 'ethernetrange.x.tcl') . "
");
