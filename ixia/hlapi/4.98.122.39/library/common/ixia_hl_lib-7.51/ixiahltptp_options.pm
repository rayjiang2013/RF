...
package ixiahltptp_options;
##
my @cmd_spec_list = (
    ['ptp_options_config', 'status', 'log', 'handles'],
    ['ptp_options_control', 'status', 'log', '*'],
    ['ptp_options_stats', 'status', 'log', '*'],
);
...
if ($@) {
  Carp::confess("ptp_options.x.tcl:
----
".ixiatcl::errorInfo()."
------
");
...
print("using " . File::Spec->join(
 File::Spec->rel2abs(File::Basename::dirname(__FILE__)), 'ptp_options.x.tcl') . "
");
