...
package ixiahltdcbxrange;
##
my @cmd_spec_list = (
    ['dcbxrange_config', 'status', 'log', 'handles'],
    ['dcbxrange_control', 'status', 'log', '*'],
    ['dcbxrange_stats', 'status', 'log', '*'],
);
...
print("using " . File::Spec->join(
 File::Spec->rel2abs(File::Basename::dirname(__FILE__)), 'dcbxrange.x.tcl') . "
");
