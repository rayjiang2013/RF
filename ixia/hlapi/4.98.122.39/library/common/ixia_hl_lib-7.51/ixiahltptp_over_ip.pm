...
package ixiahltptp_over_ip;
##
my @cmd_spec_list = (
    ['ptp_over_ip_config', 'status', 'log', 'handles'],
    ['ptp_over_ip_control', 'status', 'log', '*'],
    ['ptp_over_ip_stats', 'status', 'log', '*'],
);
...
if ($@) {
  Carp::confess("ptp_over_ip.x.tcl:
----
".ixiatcl::errorInfo()."
------
");
...
print("using " . File::Spec->join(
 File::Spec->rel2abs(File::Basename::dirname(__FILE__)), 'ptp_over_ip.x.tcl') . "
");
