...
package ixiahltvport;
##
my @cmd_spec_list = (
    ['vport_config', 'status', 'log', 'handles'],
    ['vport_control', 'status', 'log', '*'],
    ['vport_stats', 'status', 'log', '*'],
);
...
print("using " . File::Spec->join(
 File::Spec->rel2abs(File::Basename::dirname(__FILE__)), 'vport.x.tcl') . "
");
