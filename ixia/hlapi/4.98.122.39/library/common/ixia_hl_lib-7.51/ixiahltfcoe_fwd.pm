...
package ixiahltfcoe_fwd;
##
my @cmd_spec_list = (
    ['fcoe_fwd_config', 'status', 'log', 'handles'],
    ['fcoe_fwd_control', 'status', 'log', '*'],
    ['fcoe_fwd_stats', 'status', 'log', '*'],
);
...
print("using " . File::Spec->join(
 File::Spec->rel2abs(File::Basename::dirname(__FILE__)), 'fcoe_fwd.x.tcl') . "
");
