    ...
    # Else, add new interface entry.
        ...
        for {set interfaceCount 0 } { $interfaceCount < $numInterfaces} { incr interfaceCount} {
            ...
            if {[vlanUtils::isPortTagged $chassis $card $port]} {
				set vlanType [protocol cget -enable802dot1qTag]
				#errorMsg "Port: $chassis $card $port, type: $vlanType"
                if {[protocol cget -enable802dot1qTag]} {		
				    if {![vlan get $chassis $card $port]} {
					
						#errorMsg "vlanId = [vlan cget -vlanID], vlanPriority = [vlan cget -userPriority], vlanTPID = [vlan cget -protocolTagId]"
						if (![vlan cget -vlanID]) {
							if {![stackedVlan get $chassis $card $port]} {
							set numVlans [stackedVlan cget -numVlans]
							#errorMsg "stackedVlan !!! vlannr = $numVlans"
							set sVlanIds ""
							set sVlanPri ""
							set sVlanTpid ""

							for {set vlanCount 0 } { $vlanCount < $numVlans} { incr vlanCount} {
								if {$vlanCount == 0} {
									stackedVlan getFirstVlan
									set sVlanIds "[vlan cget -vlanID]"
									set sVlanPri "[vlan cget -userPriority]"
									set sVlanTpid "[vlan cget -protocolTagId]"
								} else {
									stackedVlan getNextVlan
									set sVlanIds "$sVlanIds, [vlan cget -vlanID]"
									set sVlanPri "$sVlanPri, [vlan cget -userPriority]"
									set sVlanTpid "$sVlanTpid, [vlan cget -protocolTagId]"
								}	
								#errorMsg "vlanId = [vlan cget -vlanID], vlanPriority = [vlan cget -userPriority], vlanTPID = [vlan cget -protocolTagId]"
							}
							interfaceEntry config -enableVlan       $::true
							interfaceEntry config -vlanId           $sVlanIds
							interfaceEntry config -vlanPriority     $sVlanPri
							interfaceEntry config -vlanTPID         $sVlanTpid
							} else {
