##Procedure Header
# Name:
#    ixiangpf::emulation_ldp_route_config
#
# Description:
...
#    is silently ignored. Only the fec_type option is used. If the current session or command was run with -return_detailed_handles 0 the following keys will be omitted from the command response:  fecproperty_handles, ldppwvpls_handles, ldpotherpws_handles, ldpvplsbgpad_handles
#
# See Also:
#
...
sub emulation_ldp_route_config {
	...
	# ixiahlt::logHltapiCommand('emulation_ldp_route_config', $args);
	# ixiahlt::utrackerLog ('emulation_ldp_route_config', $args);

	return ixiangpf::runExecuteCommand('emulation_ldp_route_config', \@notImplementedParams, \@mandatoryParams, \@fileParams, $args);
}
