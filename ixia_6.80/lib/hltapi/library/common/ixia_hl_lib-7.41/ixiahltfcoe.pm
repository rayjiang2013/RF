...
##
my @cmd_spec_list = (
    ['fcoe_config', 'status', 'log', 'handles'],
    ['fcoe_control', 'status', 'log', '*'],
    ['fcoe_stats', 'status', 'log', '*'],
);
...
print("using " . File::Spec->join(
 File::Spec->rel2abs(File::Basename::dirname(__FILE__)), 'fcoe.x.tcl') . "
");
