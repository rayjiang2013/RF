...
package ixiahltdcbxtlv;
##
my @cmd_spec_list = (
    ['dcbxtlv_config', 'status', 'log', 'handles'],
    ['dcbxtlv_control', 'status', 'log', '*'],
    ['dcbxtlv_stats', 'status', 'log', '*'],
);
...
print("using " . File::Spec->join(
 File::Spec->rel2abs(File::Basename::dirname(__FILE__)), 'dcbxtlv.x.tcl') . "
");
