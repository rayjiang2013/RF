...
package ixiahltiprange;
##
my @cmd_spec_list = (
    ['iprange_config', 'status', 'log', 'handles'],
    ['iprange_control', 'status', 'log', '*'],
    ['iprange_stats', 'status', 'log', '*'],
);
...
print("using " . File::Spec->join(
 File::Spec->rel2abs(File::Basename::dirname(__FILE__)), 'iprange.x.tcl') . "
");
