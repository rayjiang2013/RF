...
proc ::ixia::legacy_connect { args } {
    ...
    eval set opt_args "\{
        ...
        -ixnetwork_license_type     CHOICES perpetual subscription_tier1 subscription_tier2 subscription_tier3
                                    DEFAULT perpetual
        -tcl_server                 ANY
    ...
    if {[info exists new_ixnetwork_api] && $new_ixnetwork_api } {
        ...
        if {[isUNIX]} {
            ...
            if {![info exists tcl_server] &&\
                ...
                keylset returnList log "ERROR in $procName: When running on\
                        ...
                        provide tcl_server or ixnetwork_tcl_server."
                return $returnList
            } elseif {![info exists tcl_server]} {
                set tcl_server          [lindex [split $ixnetwork_tcl_server :] 0]
        ...
        if {[info exists master_device] && [info exists device]} {
            ...
            set master_chassis_list [lsort -nocase -unique $master_chassis_list]
    ...
    if {[info exists tcl_server]} {
        ...
        if {$hlt_init_flag} {
            ...
            set result [clientSend $::ixTclSvrHandle "set ::backup_auto_path \$::auto_path"]
            if {![info exists ::ixTclSvrHandle]} {
                keylset returnList status $::FAILURE
                keylset returnList log "Not connected to TclServer."
                return $returnList
            }
            set result [clientSend $::ixTclSvrHandle "if { !\[catch { package req registry }\] && \[catch { set ::appinfo_path \[registry get {HKEY_LOCAL_MACHINE\\SOFTWARE\\Ixia Communications\\AppInfo\\InstallInfo} HOMEDIR\] } result\] } { return -1 }"]
            if { $result == -1 } {
                ...
                set result [clientSend $::ixTclSvrHandle "set ::appinfo_cmd \[file join \$::appinfo_path appinfo.exe\]"]
                ...
                set result [clientSend $::ixTclSvrHandle "set cmd \"{\$::appinfo_cmd} --get-regkey --app-name HLTAPI --app-version $::ixia::hltapi_version\""]
                if {![info exists ::ixTclSvrHandle]} {
                    keylset returnList status $::FAILURE
                    keylset returnList log "Not connected to TclServer."
                    return $returnList
                }
                set result [clientSend $::ixTclSvrHandle "if { \[catch { set ::hltapi_regkey \[eval exec \$cmd\] } regResult\] } { return \$regResult } else { return ok }"]
                ...
                if { $result != "ok" } {
                    ...
                    set result [clientSend $::ixTclSvrHandle "if { !\[regsub \"HLTAPI: \" \$::hltapi_regkey \"\" ::hltapi_regkey\] } { return -3 }"]
                    if { $result == -3 } {
                        ...
                        set result [clientSend $::ixTclSvrHandle "if { \$::hltapi_regkey != \"\" } { set ::hltapi_path \[registry get \$::hltapi_regkey HOMEDIR\]; source \"\${::hltapi_path}TclScripts\\\\bin\\\\tcl_init.tcl\" }"]
                    }
                }
            }
        }
        ...
        if {[catch {clientSend $::ixTclSvrHandle "set env(IXIA_VERSION) $hltsetUsed"} retError]} {
            keylset returnList status $::FAILURE
        ...
        if {![catch {clientSend $::ixTclSvrHandle {package require Ixia}} retValue]} {
            ...
            catch {clientSend $::ixTclSvrHandle {package present Ixia}} retValue
            set startIndex [string last "\r" $retValue]
            if {$startIndex >= 0} {
            ...
            catch {clientSend $::ixTclSvrHandle {package require IxAccessGUI}} retValue
        }
    }
    ...
    if {$::ixia::executeOnTclServer && [info exists tcl_server] && \
        ...
        catch {clientSend $::ixTclSvrHandle "set ::tcl_interactive $::tcl_interactive"}
        ...
        foreach globalVariable $globalVariablesLst {
            if {[info exists $globalVariable]} {
                ...
                set commandToSend [format "%s" "clientSend $::ixTclSvrHandle {set \
        ...
        foreach globalVariable $globalVariablesArr {
            if {[info exists $globalVariable]} {
                ...
                set commandToSend [format "%s" "clientSend $::ixTclSvrHandle \
    ...
    if {$connect_mode == "legacy" || $connect_mode == "connect_vp_rp"} {
            ...
            if {[isIpAddressValid $single_device]} {
                ...
                if {[info exists _device($single_device)]} {
                    if {[isIpAddressValid $_device($single_device)]} {
                        ...
                        || $ip_list == ""} {
                    keylset returnList status $::FAILURE
                    keylset returnList log "ERROR in $procName: No device\
                            $single_device was found in _device."
                    return $returnList
                } else {
        ...
        if {[info exists master_device]} {
            ...
            foreach single_device $master_device {
                ...
                } elseif {[isIpAddressValid $single_device]} {
    ...
    if {[info exists new_ixnetwork_api] && $new_ixnetwork_api} {
        ...
        if {$vm_ports_detected == 1} {
            puts "WARNING: IxVM ports detected. Some operations might take longer\
                    time to complete."
        }
        
		if {$pineflag_detected == 1} {
		    puts "Warning! Port CPU Traffic configuration detected. IxTclHal API is disabled in this mode.\
				Some high level operations depending on IxTclHal might fail.\
				Please reconnect ports in order to disable this feature and restore IxTclHal functionality"
        }
...
proc ::ixia::legacy_cleanup_session { args } {
    ...
    if {[info exists new_ixnetwork_api] && ($new_ixnetwork_api == 1)} {
        ...
        set ixnetwork_variables {ixNetworkChassisConnected ixnetwork_chassis_list \
            ...
            tcl_proxy_username close_server_on_disconnect}
